		...
		if (pBlkDefRcd->hasAttributeDefinitions())
		{
			...
			for (pItr->start(); !pItr->done(); pItr->step())
			{
				...
				if (pAttDef != NULL)
				{
					...
					if (boost::algorithm::iequals(strTag, std::wstring(tag)))
					{
...
bool CBlockUtil::GetAttributeValue( AcDbBlockReference *pBlkRef, const TCHAR* tag, std::wstring &value )
{
	// 创建块表记录的属性遍历器
	bool bFound = false;
	...
	if (pItr)
	{
		while (!pItr->done())
		{
			...
			if (pBlkRef->openAttribute(pAtt, attId, AcDb::kForRead) == Acad::eOk)
			{
				...
				std::wstring strTag = tag;
				boost::algorithm::to_upper(strTag);
				if (_tcscmp(strTag.c_str(), szTag) == 0)
				{
					TCHAR *szValue = pAtt->textString();	
					value = szValue;  // std::wstring 可以直接从 TCHAR* 赋值
...
AcDbObjectId CBlockUtil::CopyBlockDefFromOtherDwg( const TCHAR* fileName, const TCHAR* blkDefName )
{
	...
	Acad::ErrorStatus es = pSourceDwg->readDwgFile(fileName);  // 移除 _SH_DENYNO 参数
	if (es != Acad::eOk)
	{
		delete pSourceDwg;
		pSourceDwg = NULL;
										   ...
										   AcDbDatabase *pDb )
{
	...
	if (blkDefId.isNull() || bOverwriteIfExist)
	{
		if (_taccess(dwgFileName, 0) != -1)
		{
			...
			Acad::ErrorStatus es = pBlkDb->readDwgFile(dwgFileName);  // 移除 _SH_DENYNO 参数
			if (es == Acad::eOk)
			{
				...
				{
					wchar_t buffer[256];
					swprintf_s(buffer, 256, L"CBlockUtil::InsertDwgBlockDef函数, insert失败:%d.", (int)es);
					acedAlert(buffer);
				}
			}
			else
			{
				wchar_t buffer[256];
				swprintf_s(buffer, 256, L"CBlockUtil::InsertDwgBlockDef函数, readDwgFile失败:%d.", (int)es);
				acedAlert(buffer);
			}
			delete pBlkDb;
		}
		...
		{
			acedAlert(TEXT("CBlockUtil::InsertDwgBlockDef函数, 未找到指定的图块文件."));
		}
	}
